...
const PullToRefresh = ({
  ...
  const { elementRef, pullDistance, isPulling } = useMobileGestures({
  ...
  const getRefreshIcon = () => {
    ...
    if (pullDistance >= threshold) {
      return <div className='refresh-icon ready'>↻</div>;
    }

    if (isPulling) {
      return <div className='refresh-icon pulling'>↻</div>;
    }
  ...
  return (
    <div ref={elementRef} className={`pull-to-refresh-container ${className}`}>
